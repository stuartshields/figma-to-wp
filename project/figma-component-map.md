...
Template parts live in `content/themes/wp-theme/template-parts/`. The theme uses **`get_extended_template_part`** (parameterised template parts: variables are passed via an array and read as `$this->vars[...]` inside the template). This is different from vanilla `get_template_part` and is important to get right.
